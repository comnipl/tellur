...
use tellur_core::timeline_component::resolve_with_canvas;
use tellur_renderer::{AudioExport, FfmpegEncoder};
...
fn main() -> ExitCode {
    ...
    // plugin's `canvas = (1920, 1080)`), then encode that same resolved tree.
    let resolved =
        match resolve_with_canvas(scene::KineticMotion::builder().build(), scene::SCENE_CANVAS) {
    ...
    let result = FfmpegEncoder::new(Resolution::new(width, height), FPS)
        // This scene has no audio; omit the otherwise-default silent track so
        // the demo keeps its original video-only output shape.
