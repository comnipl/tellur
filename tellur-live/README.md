...
cap large GPU backend allocations, including GPU-resident main-cache entries,
render targets, readback staging, and scratch buffers. Both accept byte counts
with optional `k`, `m`, or `g` suffixes, for example
`TELLUR_CACHE_RAM=1g TELLUR_VRAM=2g`.
When unset, both default to 1 GiB. `TELLUR_VRAM` is the total Tellur-managed GPU
allocation budget; GPU raster cache entries keep a separate internal byte cap
inside that total. If render-critical GPU allocation fails, that cache cap is
shrunk and frequency-selected GPU cache entries are evicted for subsequent
frames. While the GPU backend remains active and enabled, a long run of render
misses with spare VRAM grows the cap gradually again.
