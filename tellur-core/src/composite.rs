...
mod tests {
    ...
    #[test]
    fn composite_frames_recomposites_same_input_storage() {
        let frames = vec![
            RasterImage::cpu(1, 1, PixelFormat::Rgba8, vec![255, 0, 0, 255]),
            RasterImage::cpu(1, 1, PixelFormat::Rgba8, vec![0, 0, 255, 255]),
        ];
        ...
        assert_eq!(ctx.readbacks, 0);
        assert_eq!(ctx.gpu.composite_calls, 2);
        assert_eq!(ctx.gpu.composite_inputs, vec![2, 2]);
    }

    #[test]
    fn composite_frames_cpu_fallback_recomposites_same_input_storage() {
        let frames = vec![
            RasterImage::cpu(1, 1, PixelFormat::Rgba8, vec![255, 0, 0, 255]),
            RasterImage::cpu(1, 1, PixelFormat::Rgba8, vec![0, 0, 255, 255]),
        ];
