...
すべての timeline component は [`Clock`](../tellur-core/src/timeline_component/clock.rs) でサンプリングされます。`Clock` は **2つの時間軸** を公開し、絶対時刻の演出と clip-local な構成を同じ component model で扱えます。

| | global 軸 | local 軸 |
|---|---|---|
| 時間の入手方法 | `clock.global()`（`TimelineTime`） | `clock.local()` / `clock.window()`（`LocalTime` / `Window`） |
| 原点 | resolve 済み timeline のルート | この component の resolve 済み配置開始点 |
| 主な登場人物 | `TimelineTime`, `Event` | `LocalTime`, `Window`, `Placed` |
| 向いている場面 | 作品全体のモーショングラフィックス演出・共有イベント時刻 | 並べ替え・trim・配置に追従する再利用可能な clip |
...
どちらの軸でも、時間から値を作る語彙は共通です: **`Time` → `Phase` / `Window` → eased な値**。
...
`tellur-renderer/examples/timeline_to_mp4.rs` の外側のsceneはglobal軸を選び、量子化したsnapshotをraster componentへ渡します。
...
一方 `tellur-live` の `timeline_showcase` は、`clock.local()` 駆動の自己アニメと `clock.global()` 駆動の全体進行を1画面に共存させています。`demo_scene` も同じcomponent modelの中で `phase` / `window` / `clamped()` を使っています。**作品全体の演出はglobal軸で設計し、再利用可能なclipはlocal軸へ追従させる**、という使い分けです。
